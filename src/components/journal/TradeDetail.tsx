...
function Row({ label, value, mono = false, color }: { label: string; value: string | undefined; mono?: boolean; color?: string }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '7px 0', borderBottom: '1px solid var(--border-default)' }}>
      <div style={{ fontSize: 11, color: 'var(--text-tertiary)', minWidth: 140 }}>{label}</div>
      <div style={{ fontSize: 12, color: color ?? 'var(--text-primary)', textAlign: 'right', fontFamily: mono ? '"JetBrains Mono", monospace' : 'inherit', fontWeight: mono ? 500 : 400 }}>
        {value ?? '—'}
      </div>
    </div>
  );
      ...
      <div style={modal}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: '1px solid var(--border-default)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <span style={{ fontSize: 16, fontWeight: 700, color: 'var(--text-primary)', fontFamily: '"JetBrains Mono", monospace' }}>{trade.instrument}</span>
            <span style={{
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button
              ...
              style={{ padding: '6px 14px', backgroundColor: '#3D8EF0', border: 'none', borderRadius: 5, color: 'var(--text-on-accent)', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}
            >
              Edit
            </button>
            <button
              onClick={onClose}
              style={{ padding: '6px 12px', backgroundColor: 'transparent', border: '1px solid var(--border-default)', borderRadius: 5, color: 'var(--text-tertiary)', fontSize: 14, cursor: 'pointer' }}
            >
              ✕
            </button>
          </div>
        ...
        <div style={{ padding: '16px 20px', flex: 1 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 20 }}>
            {[
              ...
              { label: 'R:R', value: calc.rrRatio !== null ? `${calc.rrRatio.toFixed(2)}R` : '—', color: 'var(--text-primary)' },
              { label: 'Duration', value: fmtDuration(calc.durationMinutes), color: 'var(--text-primary)' },
            ].map((item) => (
              <div key={item.label} style={{ backgroundColor: 'var(--bg-app)', border: '1px solid var(--border-default)', borderRadius: 6, padding: '10px 12px', textAlign: 'center' }}>
                <div style={{ fontSize: 10, color: 'var(--text-tertiary)', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{item.label}</div>
          ...
          {trade.notes && (
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ backgroundColor: 'var(--bg-app)', border: '1px solid var(--border-default)', borderRadius: 6, padding: '12px 14px', fontSize: 12, color: 'var(--text-primary)', lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>
                {trade.notes}
              </div>
            </div>
          )}
          ...
          {trade.chartImageBase64 && (
            <div>
              ...
              <img
                ...
                alt="Trade chart"
                style={{ width: '100%', borderRadius: 6, border: '1px solid var(--border-default)', maxHeight: 300, objectFit: 'contain' }}
              />
            </div>
          )}
        </div>
