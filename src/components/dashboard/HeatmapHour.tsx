  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 10 }}>
        <div style={{ fontSize: 10, color: '#8892a4' }}>Loss</div>
        {[-1, -0.5, 0, 0.5, 1].map((v) => (
          <div
            key={v}
            style={{
              ...
              height: 10,
              borderRadius: 2,
              backgroundColor: v === 0 ? '#22263a' : v > 0 ? getCellColor(v, 1, 1) : getCellColor(v, 1, 1),
            }}
          />
        ))}
        <div style={{ fontSize: 10, color: '#8892a4' }}>Win</div>
        <div style={{ fontSize: 10, color: '#8892a4', marginLeft: 8 }}>| Gray = no trades</div>
      </div>
      ...
      {tooltip && (
        ...
        >
          ...
          <div>
            Avg P&L:{' '}
            <span style={{ color: tooltip.cell.avgPnl >= 0 ? '#00d17a' : '#ff4d4d' }}>
              {tooltip.cell.avgPnl >= 0 ? '+' : ''}{c}{tooltip.cell.avgPnl.toFixed(2)}
            </span>
          </div>
          <div style={{ color: '#8892a4' }}>{tooltip.cell.count} trades</div>
        </div>
      )}
    </div>
  );
